<template>
  ...
  const close = () => {
    ...
    try {
        ...
        console.log("ID del campus a actualizar:", campus.id); // Debug
        if (!campus.id) {
        ...
        await axios.put(`${API_BASE_URL}/campus/${campus.id}`, campus, {
            headers: { 
            Authorization: `Bearer ${token}`,
            'Content-Type': 'application/json'  // Importante para PUT
            }
        });
        ...
        toast.error(error.response.data.message); // Muestra el mensaje del servidor si existe
        } else {
